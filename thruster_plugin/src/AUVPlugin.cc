...
void AUV::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
    ...
    if (thrusterLink1 == NULL) {
        ROS_INFO("East thruster linsdsdk is NULL");
    }
    if (thrusterLink2 == NULL) {
        ROS_INFO("West thruster linksdds is NULL");
    }
    if (baseLink == NULL) {
    ...
    this->thrusterID = 0;
    this->thrustEfficiency = (int)(_sdf->Get<int>("thrustEfficiency"));
...
void AUV::Update() {
    ...
    double g = 9.812;
    buoyancyForce = ignition::math::Vector3d(0, 0, 1.001*mass*g);

    ignition::math::Vector3d CoB = ignition::math::Vector3d(-0.007, -0.005, 0.05-0.032);
